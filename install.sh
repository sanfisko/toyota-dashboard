...
EOF
    ...
    print_info "1. Запустите приложение:"
    print_info "   systemctl --user restart toyota-dashboard"
    echo
    print_info "2. Откройте в браузере:"
    print_info "   http://localhost:2025"
    echo
    print_info "3. Настройте Toyota данные через веб-интерфейс:"
    print_info "   - Система автоматически перенаправит на страницу настройки"
    print_info "   - Введите email, пароль и VIN номер автомобиля"
    print_info "   - Нажмите 'Проверить и сохранить настройки'"
...
main() {
    ...
    echo -e "${BLUE}╚══════════════════════════════════════════════════════════════╝${NC}"
    echo
    echo -e "${YELLOW}1. Настройте Toyota данные через веб-интерфейс:${NC}"
    echo "   http://localhost:2025/setup"
    echo "   - Введите email от Toyota Connected"
    echo "   - Введите пароль"
    echo "   - Введите VIN номер вашего Toyota автомобиля"
    echo "   - Нажмите 'Проверить и сохранить настройки'"
    echo
    echo -e "${YELLOW}2. Управление сервисом:${NC}"
    echo "   systemctl --user start toyota-dashboard    # Запуск"
    echo "   systemctl --user stop toyota-dashboard     # Остановка"
    echo "   systemctl --user restart toyota-dashboard  # Перезапуск"
    echo "   systemctl --user status toyota-dashboard   # Статус"
